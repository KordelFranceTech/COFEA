    ...
    for i in range(num_episodes):
        ...
        while not done:
            ...
            target = reward + discount_factor * np.max(
                model.best_individual.predict(np.identity(env.observation_space.n)[new_state:new_state + 1]))
            ...
            model.cofea_evolve(np.identity(env.observation_space.n)[state:state + 1],
                               target_vector.reshape(-1, env.action_space.n),
                               n_generations=n_generations)
            print(f'state: {state}\taction: {actions[
                action]}\treward: {reward}\tnew state: {new_state}\ttarget: {target}')
            state = new_state
            count += 1
